...
int main()
{
    long long int P,Q,d,D,T,mod,divd,extP,extQ=0,summ=0;
    cin>>T;
    for (long long int i = 0; i < T; i++)
    {
        ...
        //extP=D*P;
        extQ=mod*Q*divd;
        // summ=(d*(((float)divd/2)*(2*P+((divd-1)*Q))));
        summ = (P * divd *d)+(Q * (((divd-1)*(divd)/2)*d));
        cout<<summ+extQ+extP<<endl;
...
The initial rate of printing the currency is P dollars per day and they increase the production by Q dollars after every interval 
of d days. For example, after d days the rate is P+Q dollars per day, and after 2d days the rate is P+2Q dollars per day, and so on. 
